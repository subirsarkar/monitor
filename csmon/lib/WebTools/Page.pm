...
{
  my ($pkg, $params) = @_;
  croak q|URL Must be specified| unless defined $params->{url};
  my $query   = $params->{url};
  my $verbose = $params->{verbose} || 0;

  # makes an object representing the URL
  my $agent = new LWP::UserAgent(timeout => 180);
  ...
  carp q|INFO. No information found!| unless length $content;
...
sub Table
{
  ...
  my $keep_html = (defined $params->{keep_html}) ? $params->{keep_html} : 1;
  my $verbose = $params->{verbose} || 0;

  my $h = [];
  my $content = __PACKAGE__->Content({ url => $query, verbose => $verbose });
  return $h unless length $content;

  my $te = new HTML::TableExtract( depth => $depth,
                                   count => $count,
                                 gridmap => $gridmap,
                               keep_html => $keep_html);
  $te->parse($content);
