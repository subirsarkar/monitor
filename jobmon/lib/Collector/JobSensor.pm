...
use Collector::Util qw/trim avg/;
...
{
  my ($list, $value) = @_;
  return ((defined $value) ? $value : avg([split /\s+/, $list]));
}
sub dbConnected
{
  my $self = shift;
  $self->{dbconn} = new Collector::DBHandle;
...
sub correctState
{
  ...
  $query .= qq|) AND ((ceid LIKE "%$ce%")|;
  $query =~ s/OR//;
  
  # take care of local jobs also in case jobsensor is run on > CEs and _this_ is the master 
  ...
  my $iam_master = ($multi_ce) ? ((defined $master_ce and $host eq $master_ce) ? 1 : 0)
                               : 1;
  $query .= q| OR (ceid IS NULL AND user IS NOT NULL AND qtime IS NOT NULL)|
    if $iam_master;
...
sub fetch
{
  ...
  my $sthug = $dbh->prepare(q|UPDATE jobinfo_summary SET ceid=? WHERE jid=?|);
  ...
  my $sthia = $dbh->prepare(
    q|INSERT INTO jobinfo_summary 
       (jid,user,ugroup,queue,jobname,qtime,start,end,status,cputime,walltime,
          exec_host,ex_st,ceid,subject,grid_id,rb,timeleft,role,jobdesc,rank,priority)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)|
  );
  ...
  # get job status
  my $js = new Collector::JobStatus({ dbconn => $self->{dbconn} });
  ...
  while ( my ($jid) = each %$info ) {
    ...
    if ($count > 0) {
      ...
      $sthug->execute($gridce, $jid)            unless defined $ceid;
