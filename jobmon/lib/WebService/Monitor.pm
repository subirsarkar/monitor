...
sub cgiapp_init 
{
  ...
  my $monitor = new WebService::MonitorCore($cfg_href);
...
sub cgiapp_prerun
{
  ...
  $self->param( error_message => qq|Failed to retrieve the core monitoring object!| )
    and return unless defined $monitor;

  my $handler = new CGI::Untaint($self->query->Vars);
...
EOD
  ...
  $self->param( error_message => qq|Failed to access the DB handle!| )
    and return if $@;
...
{
  my ($self, $error) = @_;
  $self->header_props(  -type => qq|text/plain|, 
                      -status => qq|404 - Not Found|,
                  -connection => qq|close|
  );
  my $timenow = getTimestamp;
...
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->AuthInfo|);  
}

sub sendJobList 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->JobList|);  
}

sub sendJobList2 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->JobList2|);  
}

sub sendTagList 
{
  my $self = shift;
  $self->handler(qq|text/xml|, q|$monitor->TagList|);  
}

sub sendCompleteTagList 
{
  my $self = shift;
  $self->handler(qq|text/xml|, q|$monitor->CompleteTagList|);  
}

sub sendJobStatus 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->JobStatus|);  
}

sub sendJobInfo 
{
  my $self = shift;
  $self->handler(qq|text/xml|, q|$monitor->JobInfo|);  
}

sub sendCPULoad 
{
  my $self = shift;
  $self->handler(qq|image/png|, q|$monitor->CPULoad|);  
}

sub sendMemoryUsage 
{
  my $self = shift;
  $self->handler(qq|image/png|, q|$monitor->MemoryUsage|);  
}

sub sendPSInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->PSInfo|);  
}

sub sendTopInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->TopInfo|);  
}

sub sendWorkdirListInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->WorkdirListInfo|);  
}

sub sendJobdirListInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->JobdirListInfo|);  
}

sub sendLogInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->LogInfo|);  
}

sub sendErrorInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->ErrorInfo|);  
}

sub sendLocalIdInfo 
{
  my $self = shift;
  $self->handler(qq|text/plain|, q|$monitor->LocalIdInfo|);  
}

sub sendUserview 
{
  my $self = shift;
  $self->handler(qq|text/html|, q|$monitor->UserSummary|);  
}
