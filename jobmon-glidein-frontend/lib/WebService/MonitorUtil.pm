...
{
  my ($stime, $index) = @_;
  my @tim = gmtime($stime + 600*$index + 300); # +300: round value of label to the nearest 10 min
  $tim[1] -= $tim[1]%10;
...
{
  my ($stime, $diff) = @_;
  my @tim = gmtime($stime + $diff);
  $tim[1] -= $tim[1]%10;
  sprintf "%02d:%02d", $tim[2], $tim[1];
#  strftime qq|%H:%M|, gmtime($stime+$diff);
}
sub getTime 
{
  my $input = shift;
...
{
  my $xmlref = shift;
  my $writer = XML::Writer->new(OUTPUT => $xmlref, DATA_MODE => 'true', DATA_INDENT => 2);
...
sub parseGridmapFile
{
  ...
  my $fh = IO::File->($file, 'r');
  if (defined $fh && $fh->opened) {
    while (my $line = $fh->getline) {
      $line =~ s/"//; # remove the first quote, then split against the second
      my ($subject, $vo) = (split /"\s+/, $line);
      $vo =~ s/\.//; 
