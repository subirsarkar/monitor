...
EOF
  ...
  cat >> $path <<'EOF'
export JOBMON_CONFIG_DIR=$APPDIR/jobmon/etc/Collector
if [ -n "$PERL5LIB" ]; then
  export PERL5LIB=$APPDIR/jobmon/lib:$PERL5LIB
else
  export PERL5LIB=$APPDIR/jobmon/lib
fi
EOF
...
# Now the executable
program=\$APPDIR/jobmon/bin/$name.pl
EOF
  # switch off parameter substitution
  cat >> $path <<'EOF'
...
tmpl=$basedir/jobmon/nstall/sensor.tmpl
for name in ${list[*]}
do
